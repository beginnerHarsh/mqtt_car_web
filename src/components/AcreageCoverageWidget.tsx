...
import { Sprout, MapPin, ShieldCheck, Ruler } from 'lucide-react';
    ...
    <div className="absolute left-6 top-20 z-20 w-80 bg-slate-900/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-700/80 p-4 font-sans text-white animate-in fade-in slide-in-from-top-2 duration-300">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-extrabold text-xs tracking-tight text-white uppercase font-mono">Agri Field Coverage</h3>
            <p className="text-[10px] text-slate-400 font-medium font-mono">{selectedVehicle.deviceId} Operations</p>
          </div>
        </div>

        {/* Geofence Status Badge */}
        {activeGeofence ? (
          <span className="flex items-center gap-1 text-[10px] font-bold text-emerald-400 bg-emerald-500/20 px-2.5 py-1 rounded-full border border-emerald-500/40 shadow-sm">
            <ShieldCheck className="w-3 h-3 text-emerald-400" /> In Zone
          </span>
        ) : (
          <span className="flex items-center gap-1 text-[10px] font-bold text-slate-400 bg-slate-800/80 px-2.5 py-1 rounded-full border border-slate-700/60">
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-slate-800/60 p-3 rounded-xl border border-emerald-500/30 flex flex-col relative overflow-hidden group">
          ...
          <span className="text-xl font-black text-white font-mono mt-0.5">{acres} <span className="text-xs font-semibold text-emerald-400">Acres</span></span>
          <span className="text-[10px] font-mono text-slate-400">({hectares} Ha)</span>
        </div>

        <div className="bg-slate-800/60 p-3 rounded-xl border border-blue-500/30 flex flex-col relative overflow-hidden group">
          ...
          <span className="text-xl font-black text-white font-mono mt-0.5">{totalDistanceKm} <span className="text-xs font-semibold text-blue-400">km</span></span>
